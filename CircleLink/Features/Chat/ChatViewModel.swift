...
final class ChatViewModel: ObservableObject {
    ...
    @Published private(set) var isBlockingPeer = false
    @Published private(set) var didBlockPeer = false
    ...
    private var moderationGeneration = 0
    ...
    func blockPeer() async -> Bool {
        guard let peerUserId, let moderationRepository, !isModerating else { return false }
        moderationGeneration += 1
        let generation = moderationGeneration
        isModerating = true
        isBlockingPeer = true
        defer {
            if generation == moderationGeneration {
                isModerating = false
                isBlockingPeer = false
            }
        }
        moderationMessage = nil
        moderationErrorMessage = nil
        ...
        do {
            ...
            guard generation == moderationGeneration, !Task.isCancelled else { return false }
            didBlockPeer = true
            onPeerBlocked?()
            return true
        } catch {
    ...
    func loadMoreMessagesIfNeeded(currentIndex: Int) async {
        ...
        do {
            let fetched = try await chatRepository.fetchMessages(
                chatId: chatId,
                limit: pageSize,
                before: MessagePageCursor(createdAt: oldest.createdAt, messageId: oldest.id)
            )
